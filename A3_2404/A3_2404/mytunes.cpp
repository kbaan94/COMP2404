...
char *MyTunes::db_table_name=NULL;
MyTunes *MyTunes::mytunes=NULL;
//function to handle database queries like insert or delete
void MyTunes::executeQuery(char * sql){
	sqlite3 *db;
   char *zErrMsg = 0;
   int rc;

   rc = sqlite3_open("beatles.db", &db);
...
MyTunes::MyTunes():view(this)
{
	...
	initloadfromdb("select * from songs","songs");
	initloadfromdb("select * from recordings","recordings");
	initloadfromdb("select * from tracks","tracks");
	initloadfromdb("select * from users","users");
	initloadfromdb("select * from playlists","playlists");
	initloadfromdb("select * from playlist_tracks","playlist_tracks");
...
//function to initialize data from database;
void MyTunes::initloadfromdb(char * sql,char *tablename){
	sqlite3 *db;
   char *zErrMsg = 0;
   int rc;
   ...
   rc = sqlite3_exec(db, sql, loadCallback, 0, &zErrMsg);
    if( rc != SQLITE_OK ) {
      fprintf(stderr, "SQL error: %s\n", zErrMsg);
      sqlite3_free(zErrMsg);
   } else {
...
void MyTunes::executeAddSong(Command cmd){
	...
	if(song == NULL) return;
	string query = "insert into songs values ('"+to_string(id)+"','"+ cmd.getToken(TITLE)+"','"+cmd.getToken(COMPOSER)+"')";
	char q[query.length()+1];
...
void MyTunes::executeAddRecording(Command cmd){
	...
	string query = "insert into recordings values ( '"+to_string(id)+"','"+ cmd.getToken(TITLE)+"','"+cmd.getToken(ARTIST)+"','"+cmd.getToken(PRODUCER)+"','"+cmd.getToken(YEAR)+"')";
	char q[query.length()+1];
	strcpy(q,query.c_str());
	executeQuery(q); 
...
void MyTunes::executeAddTrack(Command cmd){
	...
	string query = "insert into tracks values ( '"+to_string(id)+"','"+ cmd.getToken(RECORDING_ID)+"','"+cmd.getToken(SONG_ID)+"','"+cmd.getToken(TRACK_NUMBER)+"')";
	char q[query.length()+1];
	strcpy(q,query.c_str());
	executeQuery(q); 
...
void MyTunes::executeAddUser(Command cmd){
	...
	string query = "insert into users values ('"+cmd.getToken(USERID)+"','"+cmd.getToken(NAME)+"')";
	char q[query.length()+1];
	strcpy(q,query.c_str());
	executeQuery(q); 
...
void MyTunes::executeAddPlaylist(Command cmd){
	...
	string query = "insert into playlists values ('"+cmd.getToken(USERID)+"','"+cmd.getToken(PLAYLIST_NAME)+"')";
	char q[query.length()+1];
	strcpy(q,query.c_str());
	executeQuery(q); 
...
void MyTunes::executeAddPlaylistTrack(Command cmd){
	...
	string query = "insert into playlist_tracks values ( 	'"+cmd.getToken(USERID)+"','"+cmd.getToken(PLAYLIST_NAME)+"','"+cmd.getToken(TRACK_ID)+"')";
	char q[query.length()+1];
	strcpy(q,query.c_str());
	executeQuery(q); 
...
void MyTunes::executeDeleteRecording(Command cmd){
	...
	string query = "delete from recordings where id = '"+cmd.getToken(ID)+"'";
	char q[query.length()+1];
	strcpy(q,query.c_str());
	executeQuery(q); 
...
void MyTunes::executeDeleteUser(Command cmd){
	...
	if(user == NULL) return;
	users.remove(*user);
	string query = "delete from users where user_id = '"+cmd.getToken(USER_ID)+"'";
	char q[query.length()+1];
	strcpy(q,query.c_str());
	executeQuery(q); 
...
void MyTunes::executeDeleteUserPlaylist(Command cmd){
	...
	string query = "delete from playlist where user_id = '"+cmd.getToken("-u")+"' and playlist_name='"+cmd.getToken("-p")+"'";
	char q[query.length()+1];
	strcpy(q,query.c_str());
	executeQuery(q); 
...
void MyTunes::executeDeleteUserPlaylistTrack(Command cmd){
  ...
  int trackIndex = stoi(trackIDstring);
	...
	string query = "delete from playlist_tracks where user_id = '"+cmd.getToken("-u")+"' and playlist_name='"+cmd.getToken("-p")+"' and tid='"+cmd.getToken("-t")+"'";
	char q[query.length()+1];
	strcpy(q,query.c_str());
	executeQuery(q); 
...
void MyTunes::executeDeleteTrack(Command cmd){
	...
	string query = "delete from tracks where tid = '"+cmd.getToken("-t")+"'";
	char q[query.length()+1];
	strcpy(q,query.c_str());
	executeQuery(q); 
	tracks.remove(*track);
...
void MyTunes::executeDeleteSong(Command cmd){
	...
	songs.remove(*song);
	string query = "delete from songs where sid = '"+cmd.getToken(ID)+"'";
	char q[query.length()+1];
	strcpy(q,query.c_str());
	executeQuery(q); 
