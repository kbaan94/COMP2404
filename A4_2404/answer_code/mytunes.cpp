...
void MyTunes::executeCMDFOLLOW(Command cmd){
	User *user1,*user2;
	Playlist *playlist;
	user1 = users.findByUserID(cmd.getToken("-u"));
	if(cmd.hasToken("stop")){
		for(auto user:users.getCollection()){
			user->removeObserver(user1,cmd.getToken("-p"));
		}
	}else{
		user2 = users.findByUserID(cmd.getToken("-f"));
		playlist = user2->findPlaylist(cmd.getToken("-p"));
	
		user2->registerObserver(user1,playlist);
	}
}
...
void MyTunes::executeAddPlaylistTrack(Command cmd){
	...
	//incase of changes notify observers
